...
router.put('/api/users/reset-lan-thi/:id', async (req, res) => {
  try {
...
router.get('/api/bai-thi', async (req, res) => {
  const baiThi = await BaiThi.find();
  res.json(baiThi);
})

router.get('/api/bai-thi/:id', async (req, res) => {
  const baiThi = await BaiThi.find({user: req.params.id});
  res.json(baiThi);
})

router.delete('/api/bai-thi/:id', async (req, res) => {
  await BaiThi.findByIdAndDelete(req.params.id);
  res.json({responsiveCode: 200});
})

router.delete('/api/bai-thi/user/:user', async (req, res) => {
